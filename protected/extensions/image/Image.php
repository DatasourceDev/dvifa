...
class Image {
     ...
     * @return  object
     */
    public function thumbnail($width, $height, $background = NULL) {
        if (!$this->valid_size('width', $width)) {
            throw new CException('image invalid width');
        }
    ...
    public function cropSquareAndResize($width) {
        if (!$this->valid_size('width', $width))
            throw new CException('image invalid width', $width);

        $this->crop(min(array($this->image['width'], $this->image['height'])), min(array($this->image['width'], $this->image['height'])))->resize($width, $width);
        return $this;
    }
