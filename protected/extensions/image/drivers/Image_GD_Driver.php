...
class Image_GD_Driver extends Image_Driver {
     ...
     * @return void
     */
    public function thumbnail($properties) {
        ...
        if (strlen($background) == 3) {
            $r = hexdec(substr($background, 0, 1) . substr($background, 0, 1));
            $g = hexdec(substr($background, 1, 1) . substr($background, 1, 1));
            $b = hexdec(substr($background, 2, 1) . substr($background, 2, 1));
        } else {
        ...
        $fill = imagecolorallocate($new, $r, $g, $b);

        imagefill($new, 0, 0, $fill);

        //compute resize ratio
        $hRatio = $height / imagesy($this->tmp_image);
        $wRatio = $width / imagesx($this->tmp_image);
        ...
        //compute sizes
        $sy = floor(imagesy($this->tmp_image) * $ratio);
        $sx = floor(imagesx($this->tmp_image) * $ratio);

        //compute margins
        //Using these margins centers the image in the thumbnail.
        //If you always want the image to the top left,
        //set both of these to 0
        $m_y = floor(($height - $sy) / 2);
        $m_x = floor(($width - $sx) / 2);

        //Copy the image data, and resample
        //
    //If you want a fast and ugly thumbnail,
        //replace imagecopyresampled with imagecopyresized
        if ($status = imagecopyresampled($new, $this->tmp_image, $m_x, $m_y, 0, 0, $sx, $sy, imagesx($this->tmp_image), imagesy($this->tmp_image))) {
            // Swap the new image for the old one
            imagedestroy($this->tmp_image);
            $this->tmp_image = $new;
